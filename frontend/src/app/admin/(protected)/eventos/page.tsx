  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-serif text-2xl font-medium text-foreground">Eventos</h1>
        <Link
      ...
      {sorted.length === 0 ? (
        <p className="mt-8 text-sm text-muted">Todavía no hay eventos. Crea el primero.</p>
      ) : (
              ...
              {sorted.map((event) => (
                <tr key={event.id} className="border-b border-border last:border-0 hover:bg-surface">
                  <td className="px-4 py-3">
                    <Link href={`/admin/eventos/${event.id}`} className="font-medium text-foreground hover:text-accent">
                      {event.title}
                    </Link>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${TONE_CLASSES[articleStatusTone(event.status)]}`}
                    >
                      {articleStatusLabel(event.status)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-muted">{formatPublishedDate(event.startsAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
