...
/**
 * Filtro de ubicación geográfica (cascada de 5 niveles) para /buscar — el
 * único lugar público que lo expone (CONTEXTO.md sección 16): los 6
 * listados públicos lo quitaron a propósito (ver category-chips.tsx), los
 * filtros finos de ubicación viven solo acá. Dirigido por la URL (no
 * estado de cliente aislado) para que los resultados filtrados sean
 * enlaces compartibles/con back del navegador, mismo criterio que el resto
 * del buscador.
 */
export function GeographyFilter({ initialChain }: { initialChain: GeographicUnit[] }) {
  ...
  function navigate(nextGeographyId: string | null) {
  ...
  async function handleSelect(levelIndex: number, unitId: string) {
    ...
    navigate(unit?.id ?? deepestSelectedId(nextSelected, levelIndex));
      ...
      <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {LEVELS.map((levelInfo, i) => {
          ...
          return (
            <select
              key={levelInfo.level}
              id={`search-filter-${levelInfo.level.toLowerCase()}`}
